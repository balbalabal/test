...
class GaussianModel(nn.Module):
    ...
    # 替换原有的 conduct_decoding 函数

    @torch.no_grad()
    def run_error_concealment_test(self, loss_rate=0.3, protected_anchor_idx=0, k_neighbors=5, logger=None):
        """
        在已解码的数据上执行内存中的错误隐藏（EC）测试。
        1. 随机选择锚点进行丢失模拟。
        2. 使用“上下文感知预测+残差插值”来修复它们。
        """
        if logger:
            logger.info(f"---[EC RUN]---: Running Error Concealment Test with {loss_rate * 100}% loss rate.")

        N = self._anchor.shape[0]
        if N == 0:
            if logger: logger.error("---[EC RUN]---: No anchors found. Aborting EC test.")
            return

        # === 1. 模拟数据丢失 ===
        # 创建一个随机掩码，True 表示“丢失”，False 表示“完好”
        lost_mask = (torch.rand(N, device=self._anchor.device) < loss_rate)
        # 保护一个锚点（例如第0个）不丢失，确保我们至少有一个邻居
        lost_mask[protected_anchor_idx] = False

        lost_indices = torch.where(lost_mask == True)[0]
        good_indices = torch.where(lost_mask == False)[0]

        num_lost = len(lost_indices)
        num_good = len(good_indices)

        if num_lost == 0:
            if logger: logger.info("---[EC RUN]---: No anchors selected for loss. Skipping interpolation.")
            return

        if num_good == 0:
            if logger: logger.error("---[EC RUN]---: All anchors are lost! Cannot interpolate. Scene will be empty.")
            # 将丢失的属性清零
            self._anchor_feat[lost_indices].zero_()
            self._scaling[lost_indices].zero_()  # 注意：这在log空间中是 -inf
            self._offset[lost_indices].zero_()
            return

        if logger:
            logger.info(
                f"---[EC RUN]---: Simulating loss for {num_lost} anchors. Using {num_good} anchors for interpolation.")

        # === 2. 准备插值 (预计算) ===
        good_anchor_coords = self._anchor[good_indices]

        # 预计算所有 "Good" 锚点的 f^h 和预测均值
        # (注意: 这里的 scaling 和 offsets 已经是解码后的值，不是原始的 _scaling)
        good_anchor_fh = self.calc_interp_feat(good_anchor_coords)
        good_anchor_mlp_output = self.get_grid_mlp(good_anchor_fh)

        # 解析 "Good" 锚点的预测均值
        good_pred_feat_mean, _, _, \
            good_pred_scaling_mean, _, \
            good_pred_offsets_mean, _, \
            _, _, _ = torch.split(good_anchor_mlp_output,
        ...
        # === 3. 执行插值 (遍历所有丢失的锚点) ===
        if logger:
            print("---[EC RUN]---: Starting interpolation loop...")

        for lost_idx in lost_indices:
            lost_coord = self._anchor[lost_idx].unsqueeze(0)  # [1, 3]

            # --- 步骤 1: 预测丢失锚点的中心值 ---
            f_h_lost = self.calc_interp_feat(lost_coord)  # [1, D_h]
            mlp_output_lost = self.get_grid_mlp(f_h_lost)  # [1, D_mlp]

            pred_feat_lost_mean, _, _, \
                pred_scaling_lost_mean, _, \
                pred_offsets_lost_mean, _, \
                _, _, _ = torch.split(mlp_output_lost,
                                      split_size_or_sections=[self.feat_dim, self.feat_dim, self.feat_dim,
                                                              6, 6,
                                                              3 * self.n_offsets, 3 * self.n_offsets,
                                                              1, 1, 1], dim=-1)

            pred_feat_lost_mean = pred_feat_lost_mean.squeeze(0)
            pred_scaling_lost_mean = pred_scaling_lost_mean.squeeze(0)
            pred_offsets_lost_mean = pred_offsets_lost_mean.view(self.n_offsets, 3)

            # --- 步骤 A: 上下文感知的邻居搜索/加权 ---
            K = k_neighbors
            NEIGHBOR_SEARCH_RANGE = max(2 * K, 20)

            distances_sq = torch.sum((good_anchor_coords - lost_coord) ** 2, dim=1)
            search_K = min(NEIGHBOR_SEARCH_RANGE, num_good)

            top_spatial_dist_sq, top_spatial_local_indices = torch.topk(distances_sq, search_K, largest=False)

            candidate_neighbor_global_indices = good_indices[top_spatial_local_indices]  # [search_K]
            candidate_neighbor_fh = good_anchor_fh[top_spatial_local_indices]  # [search_K, D_h]

            fh_similarity = torch.cosine_similarity(f_h_lost, candidate_neighbor_fh, dim=1)  # [search_K]

            spatial_weights = 1.0 / (torch.sqrt(top_spatial_dist_sq) + 1e-9)
            similarity_weights = ((fh_similarity + 1.0) / 2.0) ** 2  # beta=2

            combined_weights = spatial_weights * similarity_weights  # [search_K]

            actual_K = min(K, search_K)
            topk_combined_weights, topk_indices_in_candidates = torch.topk(combined_weights, actual_K, largest=True)

            final_neighbor_global_indices = candidate_neighbor_global_indices[topk_indices_in_candidates]  # [K]
            final_weights = topk_combined_weights / (torch.sum(topk_combined_weights) + 1e-9)  # [K] (归一化)

            # --- 步骤 2 & 3: 计算邻居残差并插值 ---

            # 获取邻居的实际解码属性
            actual_feat_neigh = self._anchor_feat[final_neighbor_global_indices]  # [K, feat_dim]
            actual_scaling_neigh = self._scaling[final_neighbor_global_indices]  # [K, 6]
            actual_offsets_neigh = self._offset[final_neighbor_global_indices]  # [K, n_offsets, 3]

            # 获取邻居的预测均值
            pred_feat_neigh_mean = good_pred_feat_mean[top_spatial_local_indices[topk_indices_in_candidates]]
            pred_scaling_neigh_mean = good_pred_scaling_mean[top_spatial_local_indices[topk_indices_in_candidates]]
            pred_offsets_neigh_mean = good_pred_offsets_mean[top_spatial_local_indices[topk_indices_in_candidates]]

            # 计算残差 (实际值 - 预测均值)
            residual_feat = actual_feat_neigh - pred_feat_neigh_mean
            residual_scaling = actual_scaling_neigh - pred_scaling_neigh_mean
            residual_offsets = actual_offsets_neigh - pred_offsets_neigh_mean.view(-1, self.n_offsets, 3)

            # 使用新的上下文感知权重 final_weights 插值 *残差*
            interp_residual_feat = torch.sum(residual_feat * final_weights.unsqueeze(1), dim=0)
            interp_residual_scaling = torch.sum(residual_scaling * final_weights.unsqueeze(1), dim=0)
            interp_residual_offsets = torch.sum(residual_offsets * final_weights.view(actual_K, 1, 1), dim=0)

            # --- 步骤 4: 最终结果 = 预测值 + 插值残差 ---
            final_feat = pred_feat_lost_mean + interp_residual_feat
            final_scaling = pred_scaling_lost_mean + interp_residual_scaling
            final_offsets = pred_offsets_lost_mean + interp_residual_offsets

            # --- 步骤 5: 用修复后的值覆盖模型中的属性 ---
            self._anchor_feat[lost_idx] = final_feat
            self._scaling[lost_idx] = final_scaling
            self._offset[lost_idx] = final_offsets

        if logger:
            logger.info(f"---[EC RUN]---: Interpolation finished. {num_lost} anchors were repaired.")
    ...
    def conduct_decoding(self, pre_path_name):
        ...
        torch.cuda.synchronize();
        t1 = time.time()
        print('Start decoding with Error Concealment (EC) support...')

        # === 1. 解码所有“关键数据” (假设它们总能成功) ===
        # 除非这些文件丢失，否则解码器无法启动
        try:
            ...
            anchor_decoded = _anchor_int_dec * voxel_size  # [N, 3]
            t_anchor += get_time() - t_anchor_0
            ...
            try:
                ...
                print(f"  WARNING [EC]: 'masks.b' not found or corrupt! Using default (all valid) masks. Error: {e}")
                masks_decoded = torch.ones((N, self.n_offsets, 1), device='cuda', dtype=torch.float32)
            t_mask += get_time() - t_mask_0

        except Exception as e:
            print(
                f"  CRITICAL ERROR: Failed to decode critical data (e.g., GPCC, Hash Grid). Cannot proceed. Error: {e}")
            return f"EC_CRITICAL_FAILURE: {e}"

        # === 2. ===
        # === PASS 1: 尝试解码所有属性批次，并标记成功/失败 ===

        steps = (N // MAX_batch_size) if (N % MAX_batch_size) == 0 else (N // MAX_batch_size + 1)

        # 存储每个批次的解码状态 (True=成功, False=丢包)
        batch_status = []

        # 创建最终的完整属性张量 (用空值初始化)
        feat_decoded_final = torch.empty(N, self.feat_dim, device='cuda', dtype=torch.float32)
        scaling_decoded_final = torch.empty(N, 6, device='cuda', dtype=torch.float32)
        offsets_decoded_final = torch.empty(N, self.n_offsets, 3, device='cuda', dtype=torch.float32)

        print(f"EC Pass 1: Detecting lost packets across {steps} batches...")
        for s in range(steps):
            ...
            # 步骤 A: 重建上下文 (这必须成功，因为它依赖于"关键数据")
            anchor_slice = anchor_decoded[N_start:N_end]
            feat_context = self.calc_interp_feat(anchor_slice)  # [N_num, ?]
            #
            mean, scale, prob, mean_scaling, scale_scaling, mean_offsets, scale_offsets, Q_feat_adj, Q_scaling_adj, Q_offsets_adj = \
                torch.split(self.get_grid_mlp(feat_context),
                            split_size_or_sections=[self.feat_dim, self.feat_dim, self.feat_dim, 6, 6,
                                                    3 * self.n_offsets, 3 * self.n_offsets, 1, 1, 1], dim=-1)
            ...
            try:
                ...
                # 将成功解码的数据填入最终张量
                feat_decoded_final[N_start:N_end] = feat_decoded
                scaling_decoded_final[N_start:N_end] = scaling_decoded.view(N_num, 6)
                offsets_decoded_final[N_start:N_end] = offsets_decoded.view(N_num, self.n_offsets, 3)
                ...
                # === 解码失败 (丢包) ===
                print(
                    f"  [Batch {s}] Packet loss detected (e.g., file not found or corrupt). Marking for interpolation.")
        ...
        batch_status_tensor = torch.tensor(batch_status, device='cuda')
        lost_batch_indices = torch.where(batch_status_tensor == False)[0]

        if len(lost_batch_indices) > 0:
            print(f"EC Pass 2: Interpolating {len(lost_batch_indices)} lost batches...")

            # 找到所有成功解码的锚点 ("Good Anchors")
            # (使用 repeat_interleave 快速将批次状态扩展到每个锚点)
            good_anchor_mask = batch_status_tensor.repeat_interleave(MAX_batch_size, dim=0)[:N]
            good_anchor_indices = torch.where(good_anchor_mask == True)[0]

            # 检查灾难性故障 (所有包都丢了)
            if len(good_anchor_indices) == 0:
                print("  ERROR [EC]: All attribute packets lost! Cannot interpolate. Filling lost data with zeros.")
                # 将所有未填充（即丢失）的锚点属性设置为0
                lost_anchor_indices = torch.where(good_anchor_mask == False)[0]
                if len(lost_anchor_indices) > 0:
                    feat_decoded_final[lost_anchor_indices].zero_()
                    scaling_decoded_final[lost_anchor_indices].zero_()
                    offsets_decoded_final[lost_anchor_indices].zero_()
            else:
                print(f"  Found {len(good_anchor_indices)} good anchors to use for interpolation.")
                good_anchor_coords = anchor_decoded[good_anchor_indices]
                lost_anchor_indices = torch.where(good_anchor_mask == False)[0]

                # 遍历所有丢失的锚点
                for lost_idx in lost_anchor_indices:
                    lost_coord = anchor_decoded[lost_idx].unsqueeze(0)  # [1, 3]

                    # 步骤 A: 找到 K 个最近的 "Good" 邻居
                    K = 5  # K-Nearest-Neighbors (可以调整)

                    # 计算到所有好邻居的平方距离
                    distances_sq = torch.sum((good_anchor_coords - lost_coord) ** 2, dim=1)

                    # 确保 K 不超过可用的 "Good" 邻居总数
                    actual_K = min(K, len(good_anchor_indices))
                    topk_dist_sq, topk_local_indices = torch.topk(distances_sq, actual_K, largest=False)

                    # 获取这些邻居在完整列表中的全局索引
                    neighbor_global_indices = good_anchor_indices[topk_local_indices]

                    # 步骤 B: 计算 IDW 权重 (逆距离加权)
                    weights = 1.0 / (torch.sqrt(topk_dist_sq) + 1e-9)  # [K]
                    weights = weights / torch.sum(weights)  # 归一化

                    # 步骤 C: 插值属性
                    # (weights 维度调整为 [K, 1] 或 [K, 1, 1] 以便广播)
                    interp_feat = torch.sum(feat_decoded_final[neighbor_global_indices] * weights.unsqueeze(1), dim=0)
                    interp_scaling = torch.sum(scaling_decoded_final[neighbor_global_indices] * weights.unsqueeze(1),
                                               dim=0)
                    interp_offsets = torch.sum(
                        offsets_decoded_final[neighbor_global_indices] * weights.view(actual_K, 1, 1), dim=0)

                    # 步骤 D: 填入最终张量 ("补洞")
                    feat_decoded_final[lost_idx] = interp_feat
                    scaling_decoded_final[lost_idx] = interp_scaling
                    offsets_decoded_final[lost_idx] = interp_offsets

        print("EC Pass 2: Decoding and interpolation finished.")
        t_total += get_time() - t_total_0
        torch.cuda.synchronize();
        t2 = time.time()
        print('Total decoding time (with EC):', t2 - t1)

        # === 4. ===
        # === 还原模型 (使用已解码和已插值的数据) ===
        # (这部分替换了原代码的)

        print('Start replacing parameters with decoded (and interpolated) ones...')

        # 我们的 final 张量现在是完整的 (N x ...)，可以直接使用
        _anchor = anchor_decoded
        _anchor_feat = feat_decoded_final
        _offset = offsets_decoded_final
        _scaling = scaling_decoded_final

        # 重建完整的 _mask 张量 ([N, 11, 1])
        _mask = torch.zeros(size=[N, self.n_offsets + 1, 1], device='cuda')
        _mask[:, :self.n_offsets, :] = masks_decoded  # 填入已解码或默认的掩码
        ...
        print('Parameters are successfully replaced by decoded (and interpolated) ones!')

        log_info = f"\nDecTime (with EC) {round(t2 - t1, 4)}"

        log_info_time = f"\nDecoded time in s (breakdown): " \
                        f"anchor {round(t_anchor, 4)}, " \
                        f"feat {round(t_feature, 4)}, " \
                        f"scaling {round(t_scaling, 4)}, " \
                        f"offsets {round(t_offset, 4)}, " \
