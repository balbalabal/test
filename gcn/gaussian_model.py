...
class GaussianModel(nn.Module):
    ...
    def conduct_decoding(self, pre_path_name):
        ...
        def _load_gnn_weights_strict(inpaintor, path):
            ...
            if meta:
                print(f"[decode] GNN meta: {meta}")

            # 强制校验：匹配太少直接报错（防止随机权重跑预测）
            if matched < max(8, len(inpaintor.state_dict()) // 4):
                raise RuntimeError("Loaded GNN weights mismatch too much. "
                                   "Train/eval backend or arch may differ. "
                                   "Please retrain with the same backend, or set HAC_GNN_BACKEND to match.")
        ...
        if use_gnn_inpaint:
            ...
            if num_missing > 0:
                ...
                # 先读取权重 meta（若存在）以决定后端/结构；环境变量可覆盖 meta
                ...
                # 后端/结构选择：env > meta > 默认
                backend = os.getenv("HAC_GNN_BACKEND", (meta.get("backend", "vanilla") if meta else "vanilla")).lower()
                k_val = int(os.getenv("HAC_GNN_K", str(meta.get("k", 16) if meta else "16")))
                h_val = int(os.getenv("HAC_GNN_H", str(meta.get("hidden", 128) if meta else "128")))
                mp_val = int(os.getenv("HAC_GNN_MP", str(meta.get("mp_iters", 1) if meta else "1")))
                conv = os.getenv("HAC_GNN_CONV", str(meta.get("conv", "edge") if meta else "edge")).lower()
                k2_val = int(os.getenv("HAC_GNN_K2", str(meta.get("k2", k_val) if meta else str(k_val))))
                heads = int(os.getenv("HAC_GNN_HEADS", str(meta.get("heads", 2) if meta else "2")))
                print(
                    f"[decode] GNN backend={backend}, k={k_val}, k2={k2_val}, hidden={h_val}, mp={mp_val}, conv={conv}, heads={heads}")

                # 构造 inpaintor（后端选择）
                if backend == "pyg":
                    from utils.pyg_inpaint import PYGInpaintor  # 仅此处用
                    inpaintor = PYGInpaintor(k=k_val, hidden=h_val, device='cuda',
                                             mp_iters=mp_val, conv_type=conv, k2=k2_val, heads=heads)
                else:
                    inpaintor = GNNInpaintor(k=k_val, hidden=h_val, device='cuda')

                # 暖启动：用小子集触发 _build（懒构建先构层）
                ...
                # 严格加载（若匹配太少直接报错，避免随机权重跑预测）
                def _load_gnn_weights_strict(inpaintor, state_or_path):
                    sd = state_or_path
                    meta_inner = None
                    if isinstance(state_or_path, str):
                        loaded = torch.load(state_or_path, map_location="cpu")
                        if isinstance(loaded, dict) and "state_dict" in loaded:
                            meta_inner = {k: v for k, v in loaded.items() if k != "state_dict"}
                    ...
                    print(
                        f"[decode] GNN load summary: matched={matched}, missing={len(res.missing_keys)}, unexpected={len(res.unexpected_keys)}")
                    if meta_inner:
                        print(f"[decode] GNN meta: {meta_inner}")
                    if matched < max(8, len(inpaintor.state_dict()) // 4):
                        raise RuntimeError(
                            "Loaded GNN weights mismatch too much. Train/eval backend or arch may differ. "
                            "Please retrain with the same backend, or set HAC_GNN_BACKEND/H/K/MP to match.")

                _load_gnn_weights_strict(inpaintor, ck)  # 直接传已加载的 ck（兼容 state_dict 或带 meta）
                ...
                # 平滑默认关闭（HAC_GNN_SMOOTH=1 才启用）
                if os.getenv("HAC_GNN_SMOOTH", "0") == "1":
                    try:
                        smoother = GNNInpaintor(device='cuda')  # 仅用于调用 smooth_missing
                    ...
                    print("[decode] smoothing disabled")

                # 标记这些位置已恢复，避免 EC 重复当作“未知”
                for _cc in range(5):
                    feat_cc_ok[_cc][miss] = True
                scaling_ok[miss] = True
                offsets_ok[miss] = True

                print(f"[decode] GNN inpaint filled {num_missing} anchors.")
        # ---------- GNN 补洞结束 ----------
