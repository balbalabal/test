    ...
    def conduct_decoding(self, pre_path_name):
        ...
        def _load_gnn_weights_strict(inpaintor, state_or_path):
            ...
            if isinstance(state_or_path, str):
                ...
                loaded = state_or_path  # 可能是 dict

            # 2) 拆出 meta + state_dict
            meta_inner = None
            sd = loaded
            ...
            if matched < max(8, len(inpaintor.state_dict()) // 4):
                raise RuntimeError(
                    ...
                    "Please ensure gnn_backend/gnn_conv/gnn_mp/gnn_k/gnn_k2/gnn_heads/gnn_hidden match training, "
                    "or re-train with these settings saved into the weight meta."
                )
