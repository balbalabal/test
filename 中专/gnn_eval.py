...
def eval_one(gaussians, scene, pipeline, bit_dir: str, model_path: str, iteration: int,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if use_gnn:
            ...
            # 结构参数（确保与训练一致）
            # 后端/消息传递/卷积类型/第二层邻域/heads 可通过额外 CLI 或外部 env 控制，这里仅透传 env，缺省不改
            # 不在这里强制设 HAC_GNN_BACKEND；默认由权重 meta 自动对齐
            # 如果需要覆盖，可在 CLI 传 --gnn_backend，main() 会用 _maybe_set_env 写入

            # 基础结构超参：仅当 CLI 传入时才覆盖
            if gnn_k is not None:
                os.environ['HAC_GNN_K'] = str(int(gnn_k))
            if gnn_hidden is not None:
                os.environ['HAC_GNN_H'] = str(int(gnn_hidden))

            # 推理资源上限：仅当 CLI 传入时才覆盖
            if gnn_max_known is not None:
                os.environ['HAC_GNN_PRED_MAX_KNOWN'] = str(int(gnn_max_known))
                os.environ['HAC_GNN_MAX_KNOWN'] = str(int(gnn_max_known))  # 兼容 vanilla
            if gnn_bq is not None:
                os.environ['HAC_GNN_PRED_BQ'] = str(int(gnn_bq))
                os.environ['HAC_GNN_BQ'] = str(int(gnn_bq))  # 兼容 vanilla

            # GNN 权重
            if gnn_weights:
                os.environ['HAC_GNN_WEIGHTS'] = os.path.abspath(gnn_weights)

            # 保守回退阈值（防止预测不稳时把画面拖崩）——可外部 env 覆盖
            os.environ.setdefault('HAC_GNN_TAU', '2.5')  # 3σ剪裁改保守
            os.environ.setdefault('HAC_GNN_ALPHA', '1.2')  # 置信更保守
            os.environ.setdefault('HAC_GNN_CONF_TH_OFF', '0.55')  # offsets 更保守
            os.environ.setdefault('HAC_GNN_CONF_TH_FEA', '0.40')
            os.environ.setdefault('HAC_GNN_CONF_TH_SCA', '0.40')

            # 平滑默认关闭（打开：HAC_GNN_SMOOTH=1；alpha 建议 0.7）
            os.environ.setdefault('HAC_GNN_SMOOTH', '0')
        else:
...
def main():
    ...
    parser.add_argument("--gnn_max_known", type=int, default=None)
    parser.add_argument("--gnn_bq", type=int, default=None)

    # 可选：结构/推理参数（不传则用 env 或默认）
    parser.add_argument("--gnn_backend", type=str, default=None, choices=[None, "vanilla", "pyg"])
    parser.add_argument("--gnn_mp", type=int, default=None)
    parser.add_argument("--gnn_conv", type=str, default=None, choices=[None, "edge", "gat"])
    parser.add_argument("--gnn_k2", type=int, default=None)
    parser.add_argument("--gnn_heads", type=int, default=None)
    parser.add_argument("--pred_max_known", type=int, default=None)
    parser.add_argument("--pred_bq", type=int, default=None)
    parser.add_argument("--tau", type=float, default=None)
    parser.add_argument("--alpha", type=float, default=None)
    parser.add_argument("--conf_off", type=float, default=None)
    parser.add_argument("--conf_fea", type=float, default=None)
    parser.add_argument("--conf_sca", type=float, default=None)
    ...
    # 把 CLI 覆盖写回 env（只在传入时）

    def _maybe_set_env(k, v):
        if v is not None:
            os.environ[k] = str(v)

    # 结构/后端
    _maybe_set_env("HAC_GNN_BACKEND", args.gnn_backend)
    _maybe_set_env("HAC_GNN_MP", args.gnn_mp)
    _maybe_set_env("HAC_GNN_K", args.gnn_k)
    _maybe_set_env("HAC_GNN_H", args.gnn_hidden)
    _maybe_set_env("HAC_GNN_K2", args.gnn_k2)
    _maybe_set_env("HAC_GNN_CONV", args.gnn_conv)
    _maybe_set_env("HAC_GNN_HEADS", args.gnn_heads)

    # PYG 推理资源
    _maybe_set_env("HAC_GNN_PRED_MAX_KNOWN", args.pred_max_known)
    _maybe_set_env("HAC_GNN_PRED_BQ", args.pred_bq)

    # 置信/阈值
    _maybe_set_env("HAC_GNN_TAU", args.tau)
    _maybe_set_env("HAC_GNN_ALPHA", args.alpha)
    _maybe_set_env("HAC_GNN_CONF_TH_OFF", args.conf_off)
    _maybe_set_env("HAC_GNN_CONF_TH_FEA", args.conf_fea)
    _maybe_set_env("HAC_GNN_CONF_TH_SCA", args.conf_sca)
