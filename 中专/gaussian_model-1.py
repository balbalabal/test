...
class GaussianModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 # --- ↓↓↓ 新增 EC 参数 (默认 None) ↓↓↓ ---
                 ec_model_dim: int = None,
                 ec_nhead: int = None,
                 ec_num_encoder_layers: int = None,
                 # (ec_num_decoder_layers 应该已被你移除)
                 ec_dim_feedforward: int = None,
                 ec_max_neighbors: int = None,
                 ec_dropout: float = None,
                 # --- ↑↑↑ 新增结束 ↑↑↑ ---
                 ):
        super().__init__()
        ...
        self.attr_dim = self.feat_dim + 6 + (self.n_offsets * 3)
        ...
        # --- ↓↓↓ [!! 新增 !!] 计算并保存 fh_dim ↓↓↓ ---
        if self.use_2D:
            len_res_3d = len(self.resolutions_list) if isinstance(self.resolutions_list, (list, tuple)) else 0
            len_res_2d = len(self.resolutions_list_2D) if isinstance(self.resolutions_list_2D, (list, tuple)) else 0
            self.fh_dim = (len_res_3d + 3 * len_res_2d) * self.n_features_per_level
        else:
            len_res_3d = len(self.resolutions_list) if isinstance(self.resolutions_list, (list, tuple)) else 0
            self.fh_dim = len_res_3d * self.n_features_per_level
        print(f"Calculated fh_dim: {self.fh_dim}")
        # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
        ...
        # --- ↓↓↓ [!! 新增 !!] 条件化创建 Transformer ↓↓↓ ---
        self.ec_transformer = None  # 默认 Transformer 为 None
        if ec_model_dim is not None and ec_model_dim > 0:
            # 确保 ec_transformer.py 在 Python 路径中
            try:
                from ec_transformer import ErrorConcealmentTransformer  # 在函数内部导入
            except ImportError:
                print(
                    "ERROR: Could not import ErrorConcealmentTransformer. Make sure ec_transformer.py is in the correct path.")
                sys.exit(1)

            print(f"Creating EC Transformer (Stage 2) with model_dim={ec_model_dim}")
            self.ec_transformer = ErrorConcealmentTransformer(
                feat_dim=self.feat_dim,
                n_offsets=self.n_offsets,
                fh_dim=self.fh_dim,  # 使用计算好的 fh_dim
                model_dim=ec_model_dim,
                nhead=ec_nhead,
                num_encoder_layers=ec_num_encoder_layers,
                dim_feedforward=ec_dim_feedforward,
                max_neighbors=ec_max_neighbors,
                dropout=ec_dropout
            ).cuda()
            print(f"Instantiated ErrorConcealmentTransformer.")
        else:
            print("EC Transformer not created (Stage 1).")
        # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
    ...
    def training_setup(self, training_args):
        ...
        # --- ↓↓↓ [!! 新增 !!] 条件化添加 Transformer ↓↓↓ ---
        if self.ec_transformer is not None:
            l.append({'params': self.ec_transformer.parameters(), 'lr': training_args.ec_transformer_lr_init,
                      "name": "ec_transformer"})
            print("Added EC Transformer parameters to optimizer.")
        # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
        ...
        # --- ↓↓↓ [!! 新增 !!] 条件化添加 Transformer 调度器 ↓↓↓ ---
        if self.ec_transformer is not None:
            self.ec_transformer_scheduler_args = get_expon_lr_func(
                lr_init=training_args.ec_transformer_lr_init,
                lr_final=training_args.ec_transformer_lr_final,
                lr_delay_mult=training_args.ec_transformer_lr_delay_mult,
                max_steps=training_args.ec_transformer_lr_max_steps
            )
            print("Setup EC Transformer LR scheduler.")
        # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            # --- ↓↓↓ [!! 新增 !!] 条件化更新 Transformer LR ↓↓↓ ---
            if self.ec_transformer is not None and param_group["name"] == "ec_transformer":
                lr = self.ec_transformer_scheduler_args(iteration)
                param_group['lr'] = lr
            # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if 'mlp' in group['name'] or 'conv' in group['name'] or 'feat_base' in group['name'] or 'encoding' in group['name']or 'ec_transformer' in group['name']:
                continue
    ...
    def save_mlp_checkpoints(self,path):
        ...
        if self.use_feat_bank:
            torch.save({
                ...
                # --- ↓↓↓ [!! 新增 (有条件) !!] ↓↓↓ ---
                **({'ec_transformer': self.ec_transformer.state_dict()} if self.ec_transformer is not None else {}),
                # --- ↑↑↑ [!! 新增 !!] ↑↑↑ ---
            }, path)
        else:
            torch.save({
    ...
    def load_mlp_checkpoints(self,path):
        ...
        # --- ↓↓↓ [!! 新增 !!] 条件化加载 Transformer ↓↓↓ ---
        if 'ec_transformer' in checkpoint and self.ec_transformer is not None:
            try:
                self.ec_transformer.load_state_dict(checkpoint['ec_transformer'])
                print("Loaded EC Transformer state dict.")
            except Exception as e:
                print(f"Failed to load EC Transformer state dict (mismatch?): {e}")
        elif self.ec_transformer is not None:
            print("WARN: EC Transformer was instantiated, but state dict not found in checkpoint (Stage 2 init?).")
        elif 'ec_transformer' in checkpoint:
            print("INFO: Checkpoint contains EC Transformer, but it was not instantiated (Stage 1).")
        # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
    ...
    @torch.no_grad()
    def conduct_decoding_v2(self, pre_path_name):
        """
        重构的解码函数 (V3 - 最终修复版)
        - 修复了文件名生成逻辑，以精确匹配 '..._s_cc.b' 和 '..._s.b' 格式。
        - 采用两遍法来正确处理 EC。
        """
        t_start = get_time()
        print(f'Start decoding (V3 - Final Fix) from: {pre_path_name}')
        ...
        masks_b_name = os.path.join(pre_path_name, 'masks.b')
        masks_decoded = decoder(N * self.n_offsets, masks_b_name).view(-1, self.n_offsets, 1)

        hash_b_name = os.path.join(pre_path_name, 'hash.b')
        if self.ste_binary:
            N_hash = torch.zeros_like(self.get_encoding_params()).numel()
            hash_embeddings = decoder(N_hash, hash_b_name)
        ...
        if self.ec_transformer is not None:
            try:
                ...
                all_anchor_pos_np = anchor_decoded.detach().cpu().numpy().astype('float32')
                gpu_index.add(all_anchor_pos_np)
                faiss_initialized = True
                print(f"Faiss GPU index initialized and populated with {gpu_index.ntotal} anchors.")
            except Exception as e:
                print(f"Failed to initialize Faiss GPU index: {e}. EC will fail.")

        # 初始化空的完整张量
        feat_decoded = torch.zeros(size=[N, self.feat_dim], device='cuda', dtype=torch.float32)
        scaling_decoded = torch.zeros(size=[N, 6], device='cuda', dtype=torch.float32)
        offsets_decoded = torch.zeros(size=[N, self.n_offsets, 3], device='cuda', dtype=torch.float32)

        # 初始化丢失掩码
        missing_feat_mask = torch.zeros(size=[N], device='cuda', dtype=torch.bool)
        missing_scaling_mask = torch.zeros(size=[N], device='cuda', dtype=torch.bool)
        missing_offsets_mask = torch.zeros(size=[N], device='cuda', dtype=torch.bool)

        if self.ec_transformer is not None:
            k_neighbors = self.ec_transformer.max_neighbors
            split_sections = [self.feat_dim, self.feat_dim, self.feat_dim, 6, 6, 3 * self.n_offsets, 3 * self.n_offsets,
                              1, 1, 1]
            Q_feat_base = 1.0
        ...
        steps = (N // MAX_batch_size) if (N % MAX_batch_size) == 0 else (N // MAX_batch_size + 1)
        print(f"Total anchors: {N}. Batch size: {MAX_batch_size}. Steps: {steps}.")  # 确认 steps

        # --- [!! 第一遍：解码所有可用文件 !!] ---
        print("--- Starting Pass 1: Decoding all available files... ---")
        for s in range(steps):
            N_start = s * MAX_batch_size
            N_end = min((s + 1) * MAX_batch_size, N)
            N_num = N_end - N_start
            ...
            # --- [!! 关键修复 V4: 正确的文件名逻辑 !!] ---
            # 1. 定义基本文件名 (不含 .b)
            feat_b_name_base = os.path.join(pre_path_name, f'feat_{s}')
            scaling_b_name_base = os.path.join(pre_path_name, f'scaling_{s}')
            offsets_b_name_base = os.path.join(pre_path_name, f'offsets_{s}')

            # 2. 构建 *正确* 的文件名 (匹配 feat_35_4_0.b)
            feat_files_to_check = [f'{feat_b_name_base}_{cc}_0.b' for cc in range(5)]
            scaling_file_to_check = f'{scaling_b_name_base}_0.b'
            offsets_file_to_check = f'{offsets_b_name_base}_0.b'

            # 3. 检查文件是否存在 (并打印)
            # --- ↓↓↓ [!! 你的要求：添加日志 !!] ---
            print(f"[Pass 1, s={s}] Checking for files...")
            feat_files_missing = []
            for f in feat_files_to_check:
                print(f"    -> Searching for: {f}")
                feat_files_missing.append(not os.path.exists(f))

            print(f"    -> Searching for: {scaling_file_to_check}")
            scaling_file_missing = not os.path.exists(scaling_file_to_check)
            print(f"    -> Searching for: {offsets_file_to_check}")
            offsets_file_missing = not os.path.exists(offsets_file_to_check)
            # --- ↑↑↑ [!! 你的要求：添加日志 !!] ---
            # --- [!! 修复结束 V4 !!] ---

            # 4. 获取上下文 (MLPs)
            anchor_slice = anchor_decoded[N_start:N_end]
            feat_context = self.calc_interp_feat(anchor_slice)
            mean, scale, prob, mean_scaling, scale_scaling, mean_offsets, scale_offsets, Q_feat_adj, Q_scaling_adj, Q_offsets_adj = \
                torch.split(self.get_grid_mlp(feat_context), split_size_or_sections=split_sections, dim=-1)
            ...
            scale_scaling = torch.clamp(scale_scaling.contiguous().view(-1), min=1e-9)
            scale_offsets = torch.clamp(scale_offsets.contiguous().view(-1), min=1e-9)
            Q_feat = Q_feat_base * (1 + torch.tanh(Q_feat_adj))
            Q_scaling = Q_scaling_base * (1 + torch.tanh(Q_scaling_adj))
            Q_offsets = Q_offsets_base * (1 + torch.tanh(Q_offsets_adj))

            # 5. 解码 Features
            if any(feat_files_missing):
                missing_feat_mask[N_start:N_end] = True
            else:
                mean_scale = torch.cat([mean, scale, prob], dim=-1)
                scale = torch.clamp(scale, min=1e-9)
                batch_feat_decoded = torch.zeros(size=[N_num, self.feat_dim], device='cuda', dtype=torch.float32)
                for cc in range(5):
                    chunk_file_name = feat_files_to_check[cc]  # <--- [!! 使用正确的文件名 !!]
                    mean_adj, scale_adj, prob_adj = self.get_deform_mlp.forward(batch_feat_decoded, mean_scale,
                                                                                to_dec=cc)
                    probs = torch.stack([prob[:, cc * 10:cc * 10 + 10], prob_adj], dim=-1)
                    probs = torch.softmax(probs, dim=-1)
                    Q_feat_tmp = Q_feat[:, cc * 10:cc * 10 + 10].contiguous().view(-1)
                    ...
                    feat_decoded_tmp = decoder_gaussian_mixed_chunk(
                        ...
                        [scale[:, cc * 10:cc * 10 + 10].contiguous().view(-1), scale_adj.contiguous().view(-1)],
                        [probs[..., 0].contiguous().view(-1), probs[..., 1].contiguous().view(-1)],
                        Q_feat_tmp, file_name=chunk_file_name, chunk_size=50_0000)

                    print(f"    [Pass 1, s={s}] Successfully DECODED file: {chunk_file_name}")
                    batch_feat_decoded[:, cc * 10:cc * 10 + 10] = feat_decoded_tmp.view(N_num, 10)
                feat_decoded[N_start:N_end] = batch_feat_decoded

            # 6. 解码 Scaling
            if scaling_file_missing:
                missing_scaling_mask[N_start:N_end] = True
            else:
                batch_scaling_decoded = decoder_gaussian_chunk(mean_scaling, scale_scaling, Q_scaling,
                                                               file_name=scaling_file_to_check,
                                                               chunk_size=10_0000)  # <--- [!! 使用正确的文件名 !!]
                print(f"    [Pass 1, s={s}] Successfully DECODED file: {scaling_file_to_check}")
                scaling_decoded[N_start:N_end] = batch_scaling_decoded.view(N_num, 6)

            # 7. 解码 Offsets
            if offsets_file_missing:
                missing_offsets_mask[N_start:N_end] = True
            else:
                masks_tmp = masks_decoded[N_start:N_end].repeat(1, 1, 3).view(-1, 3 * self.n_offsets).view(-1).to(
                    torch.bool)
                offsets_decoded_tmp = decoder_gaussian_chunk(mean_offsets[masks_tmp], scale_offsets[masks_tmp],
                                                             Q_offsets[masks_tmp], file_name=offsets_file_to_check,
                                                             # <--- [!! 使用正确的文件名 !!]
                                                             chunk_size=10_0000)
                print(f"    [Pass 1, s={s}] Successfully DECODED file: {offsets_file_to_check}")
                batch_offsets_decoded = torch.zeros_like(mean_offsets, device='cuda')
                batch_offsets_decoded[masks_tmp] = offsets_decoded_tmp
                offsets_decoded[N_start:N_end] = batch_offsets_decoded.view(N_num, -1).view(N_num, self.n_offsets, 3)

            torch.cuda.empty_cache()
        print("--- Pass 1 Complete. Decoded all available files. ---")

        # --- [!! 第二遍：修复所有丢失文件 !!] ---
        if self.ec_transformer is not None:
            all_missing_indices = torch.where(missing_feat_mask | missing_scaling_mask | missing_offsets_mask)[0]
            num_missing = len(all_missing_indices)

            if num_missing == 0:
                print("--- Pass 2: No files missing, EC Transformer not required. ---")
            else:
                print(f"--- Starting Pass 2: Fixing {num_missing} missing anchors using EC Transformer... ---")

                attrs_q_pass1 = torch.cat([
                    feat_decoded,
                    scaling_decoded,
                    offsets_decoded.reshape(N, -1)
                ], dim=1).detach()  # [N, attr_dim]

                knn_batch_size = 2048
                for i in tqdm(range(0, num_missing, knn_batch_size), desc="EC Fixing Progress"):
                    batch_indices = all_missing_indices[i: i + knn_batch_size]
                    B = len(batch_indices)

                    batch_missing_mask = torch.stack([
                        missing_feat_mask[batch_indices],
                        missing_scaling_mask[batch_indices],
                        missing_offsets_mask[batch_indices]
                    ], dim=1)  # [B, 3]

                    anchor_slice = anchor_decoded[batch_indices]
                    feat_context = self.calc_interp_feat(anchor_slice)

                    neighbor_indices, neighbor_padding_mask = find_k_nearest_neighbors(
                        batch_indices, anchor_decoded, k=k_neighbors,
                        gpu_index=gpu_index, faiss_initialized=faiss_initialized
                    )

                    neighbor_attrs_q = attrs_q_pass1[neighbor_indices]  # [B, k, attr_dim]

                    predicted_attrs = self.ec_transformer(
                        anchor_slice.detach(),
                        feat_context.detach(),
                        torch.zeros((B, self.attr_dim), device='cuda'),
                        batch_missing_mask,
                        anchor_decoded[neighbor_indices].detach(),
                        neighbor_attrs_q.detach(),
                        neighbor_padding_mask
                    )

                    if batch_missing_mask[:, 0].any():  # 修复 Feat
                        mask = batch_missing_mask[:, 0]
                        feat_decoded[batch_indices[mask]] = predicted_attrs[mask, :self.feat_dim]
                    if batch_missing_mask[:, 1].any():  # 修复 Scaling
                        mask = batch_missing_mask[:, 1]
                        scaling_decoded[batch_indices[mask]] = predicted_attrs[mask, self.feat_dim: self.feat_dim + 6]
                    if batch_missing_mask[:, 2].any():  # 修复 Offsets
                        mask = batch_missing_mask[:, 2]
                        offsets_decoded[batch_indices[mask]] = predicted_attrs[mask, self.feat_dim + 6:].view(
                            mask.sum(), self.n_offsets, 3)

                print("--- Pass 2 Complete. All missing anchors fixed by EC Transformer. ---")

        # --- [!! 最终填充 !!] ---
        print('Start replacing parameters with decoded ones...')

        # [!! 修复 Rotation !!]
        rotation_decoded = torch.zeros((N, 4), device="cuda")
        rotation_decoded[:, 0] = 1.0  # 默认 (1,0,0,0) 四元数
        ...
        self._mask = nn.Parameter(masks_decoded.float())  # [!! 修复 dtype !!]
        self._rotation = nn.Parameter(rotation_decoded)
        ...
        log_info = f"\nDecTime V3 (Final Fix) {round(t_total, 4)}"
        return log_info
    ...
    def conduct_decoding(self, pre_path_name):
        ...
        # --- ↓↓↓ [!! 新增 !!] 初始化 Faiss 索引 ↓↓↓ ---
        if self.ec_transformer is not None:  # 仅在 Transformer 存在时初始化
            try:
                res = faiss.StandardGpuResources()
                index_flat = faiss.IndexFlatL2(3)
                gpu_index = faiss.index_cpu_to_gpu(res, 0, index_flat)
                faiss_initialized = True
                all_anchor_pos_np = anchor_decoded.detach().cpu().numpy().astype('float32')
                gpu_index.add(all_anchor_pos_np)  # 填充索引
                print(f"Faiss GPU index initialized and populated with {gpu_index.ntotal} anchors.")
            except Exception as e:
                print(f"Failed to initialize Faiss GPU index: {e}. EC will fail.")
                faiss_initialized = False
        ...
        # --- ↓↓↓ [!! 新增 !!] 从 train_ec.py 复制 EC 训练参数/逻辑 ↓↓↓ ---
        if self.ec_transformer is not None:
            k_neighbors = self.ec_transformer.max_neighbors
            split_sections = [
        ...
        for s in range(steps):
            ...
            # --- ↓↓↓ [!! 新增 !!] 检查文件并准备 EC ↓↓↓ ---
            feat_files_missing = [not os.path.exists(feat_b_name.replace('.b', f'_{cc}.b')) for cc in range(5)]
            scaling_file_missing = not os.path.exists(scaling_b_name)
            offsets_file_missing = not os.path.exists(offsets_b_name)

            predicted_attrs = None  # 初始化
            if (any(feat_files_missing) or scaling_file_missing or offsets_file_missing) and self.ec_transformer is not None:
                print(f"--- [Batch {s}] Missing files detected! Running EC Transformer. ---")

                target_missing_mask = torch.tensor([[
                    any(feat_files_missing),
                    scaling_file_missing,
                    offsets_file_missing
                ]], device='cuda').expand(N_num, -1)  # [B, 3]

                # --- 复制 train_ec.py 中的上下文收集逻辑 ---
                target_indices = torch.arange(N_start, N_end, device='cuda')
                target_pos = anchor_sort
                target_fh = feat_context

                # (1. KNN 查找)
                neighbor_indices, neighbor_padding_mask = find_k_nearest_neighbors(
                    target_indices, anchor_decoded, k=k_neighbors,
                    gpu_index=gpu_index, faiss_initialized=faiss_initialized
                )

                # (2. 获取邻居属性 - 简化版)
                # (在解码时，我们无法知道邻居的“真实”属性，只能用0或默认值)
                neighbor_pos = anchor_decoded[neighbor_indices]
                neighbor_attrs_q = torch.zeros((N_num, k_neighbors, self.attr_dim), device='cuda')

                # (3. 调用 Transformer)
                predicted_attrs = self.ec_transformer(
                    target_pos.detach(),
                    target_fh.detach(),
                    torch.zeros((N_num, self.attr_dim), device='cuda'),  # 部分属性用 0 填充
                    target_missing_mask,
                    neighbor_pos.detach(),
                    neighbor_attrs_q,
                    neighbor_padding_mask
                )
                print(f"--- [Batch {s}] EC Transformer prediction complete. ---")
            # --- ↑↑↑ [!! 新增 !!] EC 准备结束 ↑↑↑ ---
            t_feature_0 = get_time()
            feat_decoded = torch.zeros(size=[N_num, self.feat_dim], device='cuda', dtype=torch.float32)
            if any(feat_files_missing):
                if predicted_attrs is not None:
                    print(f"Batch {s}: Concealing 'feat' with Transformer output.")
                    feat_decoded = predicted_attrs[:, :self.feat_dim]  # 使用预测的 feat
                else:
                    print(f"Batch {s}: 'feat' file missing but EC Transformer failed or disabled. Using zeros.")
            else:
                # 正常解码
                mean_scale = torch.cat([mean, scale, prob], dim=-1)
                scale = torch.clamp(scale, min=1e-9)
                for cc in range(5):
                    chunk_file_name = feat_b_name.replace('.b', f'_{cc}.b')
                    ...
                    feat_decoded_tmp = decoder_gaussian_mixed_chunk(
                        ...
                        [probs[..., 0].contiguous().view(-1), probs[..., 1].contiguous().view(-1)],
                        Q_feat_tmp,
                        file_name=chunk_file_name, chunk_size=50_0000)
            ...
            if scaling_file_missing:
                if predicted_attrs is not None:
                    print(f"Batch {s}: Concealing 'scaling' with Transformer output.")
                    scaling_decoded = predicted_attrs[:, self.feat_dim: self.feat_dim + 6]
                else:
                    print(f"Batch {s}: 'scaling' file missing but EC Transformer failed or disabled. Using zeros.")
                    scaling_decoded = torch.zeros((N_num, 6), device='cuda')
            else:
                # 正常解码
                scaling_decoded = decoder_gaussian_chunk(mean_scaling, scale_scaling, Q_scaling,
                                                         file_name=scaling_b_name, chunk_size=10_0000)
            ...
            if offsets_file_missing:
                if predicted_attrs is not None:
                    print(f"Batch {s}: Concealing 'offsets' with Transformer output.")
                    offsets_decoded_flat = predicted_attrs[:, self.feat_dim + 6:]  # 提取 offset
                    offsets_decoded = offsets_decoded_flat.view(N_num, self.n_offsets, 3)  # Reshape
                else:
                    print(f"Batch {s}: 'offsets' file missing but EC Transformer failed or disabled. Using zeros.")
                    offsets_decoded = torch.zeros((N_num, self.n_offsets, 3), device='cuda')
            else:
                # 正常解码
                offsets_decoded_tmp = decoder_gaussian_chunk(mean_offsets[masks_tmp], scale_offsets[masks_tmp],
                                                             Q_offsets[masks_tmp], file_name=offsets_b_name,
                                                             chunk_size=10_0000)
                offsets_decoded = torch.zeros_like(mean_offsets, device='cuda')
