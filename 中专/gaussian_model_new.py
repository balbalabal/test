...
class GaussianModel(nn.Module):
    ...
    def conduct_encoding(self, pre_path_name):
        ...
        M = int(os.getenv("HAC_INTERLEAVE_M", "32"))
    ...
    def conduct_decoding(self, pre_path_name):
        ...
        # 读 meta，判定是否 interleave_v1
        meta_path = os.path.join(pre_path_name, "interleave_meta.json")
        use_interleave = os.path.exists(meta_path)
        ...
        if self.ste_binary:
            ...
            hash_embeddings = decoder(N_hash, os.path.join(pre_path_name, 'hash.b'))  # {0,1}
        ...
        if not use_interleave:
            # === 原始解码逻辑（略） ===
            # 保持你当前文件里的原 conduct_decoding 内容（非 interleave 部分）。
            # 为简洁，这里不重复粘贴；如果你想彻底替换，请把你现有文件中“非 interleave”的部分留在此分支。
            raise RuntimeError("interleave_meta.json not found; keep original decoding branch here.")
        else:
            # === interleave 解码逻辑 ===
            ...
            # max_batch_size = meta.get("max_batch_size", MAX_batch_size)  # 可不使用
            ...
            for s in range(steps):
                ...
                for m in range(M):
                    ...
                    # 先解 masks（每锚点 n_offsets 个）
                    # 若文件不存在，说明 B 应为 0；但我们已检查 B>0，所以应存在
                    ...
                    # 计算该 lane 子批上下文
                    anchor_slice = anchor_step[idx_local]
                    feat_context = self.calc_interp_feat(anchor_slice)
                    mean, scale, prob, mean_scaling, scale_scaling, mean_offsets, scale_offsets, \
                    Q_feat_adj, Q_scaling_adj, Q_offsets_adj = torch.split(
                        self.get_grid_mlp(feat_context),
                    ...
                    Q_scaling_adj = Q_scaling_adj.contiguous().repeat(1, mean_scaling.shape[-1]).view(-1) # [B*6]
                    Q_offsets_adj = Q_offsets_adj.contiguous().repeat(1, mean_offsets.shape[-1]).view(-1) # [B*3*n_offsets]
                    Q_feat_t = (1.0 * (1 + torch.tanh(Q_feat_adj)))
                    Q_scaling_t = (0.001 * (1 + torch.tanh(Q_scaling_adj)))
                    Q_offsets_t = (0.2 * (1 + torch.tanh(Q_offsets_adj)))
                    ...
                    for cc in range(5):
                        ...
                        dec = decoder_gaussian_mixed_chunk(
                            ...
                            [scale[:, cc * 10:cc * 10 + 10].contiguous().view(-1), scale_adj.contiguous().view(-1)],
                            [probs[..., 0].contiguous().view(-1), probs[..., 1].contiguous().view(-1)],
                            Q_feat_t[:, cc * 10:cc * 10 + 10].contiguous().view(-1),
                            file_name=feat_file,
                            chunk_size=500_000
                        )
            ...
            # 将解码结果写回 self（decoded_version 模式）
            # 只在最后一次 step 后写回，避免频繁赋值
            # 注意 _mask 仍为 [N, n_offsets+1, 1]，这里只填前 n_offsets，最后一维置 0
