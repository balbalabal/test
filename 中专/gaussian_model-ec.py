...
from utils.encodings import \
    ...
    anchor_round_digits, \
    get_binary_vxl_size
from scene.ec_head import ECHead
...
class GaussianModel(nn.Module):
    ...
    def __init__(self,
        ...
        ctx_dim = self.encoding_xyz.output_dim + 12  # 固定 12 维签名
        self.ec_head = ECHead(ctx_dim=ctx_dim, hid=128, attn_dim=64, num_heads=4, n_offsets=n_offsets).cuda()
        self.ec_head.eval()
    ...
    def load_mlp_checkpoints(self, path):
        ...
        # 兼容旧结构：仅加载 shape 匹配的张量
        def _load_partial(module: torch.nn.Module, sd: dict, module_name="ec_head"):
            ...
            if len(skipped) > 0:
                print(f"[{module_name}] partially loaded: {len(filtered)} tensors; skipped {len(skipped)} keys.")

        if 'ec_head' in checkpoint:
            try:
                _load_partial(self.ec_head, checkpoint['ec_head'], module_name="ec_head")
            except Exception as e:
                print(f"[ec_head] partial load failed: {e}. keep current init.")
    ...
    def conduct_decoding(self, pre_path_name):
        ...
        # pre_path_name 是 bitstreams 目录；其父目录就是 model_root
        model_root = os.path.abspath(os.path.join(pre_path_name, os.pardir))
        bit_mlp = os.path.join(pre_path_name, "mlp_ckpt.pth")  # 若 encoding 时按建议保存了这份
        loaded_from = None
        if os.path.isfile(bit_mlp):
            try:
                ...
                print(f"[decode] WARNING: load_mlp_checkpoints failed: {e}; keep current ec_head.")

        if loaded_from is None:
            # fallback: 加载训练期保存的最新 checkpoint.pth
            latest_ck = _find_latest_training_mlp_ckpt(model_root)
            if latest_ck is not None and os.path.isfile(latest_ck):
                try:
                    self.load_mlp_checkpoints(latest_ck)
                    loaded_from = latest_ck
                    print(f"[decode] Loaded MLP/encoding weights from {latest_ck}")
                except Exception as e:
                    raise RuntimeError(f"[decode] load_mlp_checkpoints failed: {e}")
            else:
                raise RuntimeError(
                    f"[decode] No MLP checkpoint found: neither {bit_mlp} nor point_cloud/iteration_*/checkpoint.pth")
        ...
        missing_records = []  # 记录缺失文件（按 lane/step/attr）
            ...
            for m in range(M):
                ...
                if os.path.exists(masks_file):
                    try:
                        ...
                        if not masks_m.is_cuda:
                            masks_m = masks_m.to('cuda')
                ...
                Q_feat_rep = Q_feat_adj.contiguous().repeat(1, mean.shape[-1])  # [B, F]
                Q_scaling_vec = (0.001 * (1 + torch.tanh(Q_scaling_adj.contiguous()
                                                         .repeat(1, mean_scaling.shape[-1]).view(-1))))  # [B*6]
                Q_offsets_vec = (0.2 * (1 + torch.tanh(Q_offsets_adj.contiguous()
                                                       .repeat(1, mean_offsets.shape[-1]).view(-1))))  # [B*3*K]
        ...
        # 预计算 offsets 残差均值/“std”（仅 True 位置），以及 feat/scal 残差 mean/std
        off_pred_all = mean_offs_all.view(N, self.n_offsets, 3)
        off_res_abs_all = (offsets_decoded - off_pred_all).abs()
        mt_all = masks_decoded.to(torch.bool).expand_as(off_res_abs_all)
        num_off = (off_res_abs_all * mt_all).sum(dim=(1, 2))
        den_off = mt_all.to(torch.float32).sum(dim=(1, 2)).clamp_min(1.0)
        off_res_mean = (num_off / den_off).unsqueeze(1)  # [N,1]
        # 简化 off_res_std 为 mean 的替代；若需更准可改为 Ex2 -(E)^2 的开方
        off_res_std = off_res_mean.clone()

        feat_res_full = (feat_decoded - mean_all).view(N, 5, 10)
        feat_res_mu = feat_res_full.mean(dim=2)  # [N,5]
        feat_res_std = feat_res_full.std(dim=2)  # [N,5]
        scal_res_full = (scaling_decoded - mean_scal_all)  # [N,6]
        scal_res_mu = scal_res_full.mean(dim=1, keepdim=True)  # [N,1]
        scal_res_std = scal_res_full.std(dim=1, keepdim=True)  # [N,1]

        fh_dim = feat_ctx_all.shape[1]
        ctx_in = int(self.ec_head.q_enc[0].in_features)
        assert ctx_in - fh_dim == 12, f"EC ctx dim mismatch: expected fh+12, got ctx_in={ctx_in}, fh_dim={fh_dim}"

        # KNN 超参（可用 env 覆盖）
        K = int(os.getenv("HAC_EC_K", "8"))  # 最终邻居数
        L = int(os.getenv("HAC_EC_L", "128"))  # 空间预筛邻居数（L>=K）
        Bm = int(os.getenv("HAC_EC_MB", "1024"))  # 缺失批大小
        AvC = int(os.getenv("HAC_EC_AVC", "8192"))  # 可用集分块大小
        beta = 2.0  # 相似度权重指数
        K = max(1, K)
        L = max(K, L)
        ...
        def knn_residual_interp(miss_idx: torch.Tensor,
                                avail_idx: torch.Tensor,
                                pred_ref: torch.Tensor,  # [N, Dref]
                                actual_ref: torch.Tensor,  # [N, Dref]
                                mask_true: torch.Tensor = None,  # [N, ...] bool，用于 offsets True 位
                                ref_shape: tuple = None):  # 恢复形状
            """在 miss_idx 上用 avail_idx 的残差做插值；pred_ref 是预测均值，actual_ref 是已解码真值。"""
            Mm = miss_idx.numel()
            if Mm == 0:
                return None
            if avail_idx.numel() == 0:
                pred = pred_ref[miss_idx]
                if mask_true is None:
                    return pred
                else:
                    mt = mask_true[miss_idx].to(pred.dtype).expand_as(pred)  # [B,Koff,3] broadcast
                    return pred * mt

            Ma = avail_idx.numel()
            result = torch.empty((Mm,) + pred_ref.shape[1:], device='cuda', dtype=torch.float32)

            pos_miss_all = pos_all[miss_idx]
            pos_av_all = pos_all[avail_idx]

            for ms in range(0, Mm, Bm):
                me = min(ms + Bm, Mm)
                B = me - ms
                miss_slice = miss_idx[ms:me]
                pos_miss = pos_miss_all[ms:me]
                fh_miss_n = fh_all_n[miss_slice]

                # 空间 Top-L 预筛（对 avail 分块）
                topd = torch.full((B, L), float('inf'), device='cuda')
                topi = torch.full((B, L), -1, dtype=torch.long, device='cuda')

                for as_ in range(0, Ma, AvC):
                    ae = min(as_ + AvC, Ma)
                    pos_av = pos_av_all[as_:ae]
                    d = torch.cdist(pos_miss, pos_av)  # [B,Ac]
                    ...
                    topd, topi = newd, newi
                    del d, d_all, i_all, newd, newk, newi

                nn_idx_L = avail_idx[topi]  # [B,L]

                # 联合权重：空间 * 特征相似度
                spatial_w = 1.0 / torch.sqrt(torch.clamp(topd, min=1e-9))
                fh_nei = fh_all_n[nn_idx_L.view(-1)].view(B, L, -1)
                sim = (fh_nei * fh_miss_n.unsqueeze(1)).sum(dim=-1).clamp(-1, 1)  # [B,L]
                sim_w = ((sim + 1.0) * 0.5) ** beta
                comb_w = spatial_w * sim_w

                combK, idxK = torch.topk(comb_w, k=min(K, L), largest=True)
                nn_idx_K = nn_idx_L.gather(1, idxK)
                w = combK / torch.clamp(combK.sum(dim=1, keepdim=True), min=1e-9)  # [B,K]

                # 残差插值
                neigh_act = actual_ref[nn_idx_K.view(-1)].view((B, -1) + pred_ref.shape[1:])
                neigh_mu = pred_ref[nn_idx_K.view(-1)].view((B, -1) + pred_ref.shape[1:])
                r = neigh_act - neigh_mu  # [B,K,...]
                # 加权和
                while w.dim() < r.dim():
                    w = w.unsqueeze(-1)
                r_hat = (w * r).sum(dim=1)  # [B,...]

                rec = pred_ref[miss_slice] + r_hat  # [B,...]
                if mask_true is not None:
                    mt = mask_true[miss_slice].to(rec.dtype).expand_as(rec)  # [B,Koff,3]
                    rec = rec * mt
                result[ms:me] = rec

            return result

        # 先拷贝 feat_decoded 为工作副本，按 cc 顺序逐段填补缺失
        # ===== 新增: 学习型 EC 辅助函数 =====
        use_ec_head = int(os.getenv("HAC_USE_EC_HEAD", "0")) > 0

        off_pred_all = mean_offs_all.view(N, self.n_offsets, 3)  # [N,Koff,3]
        off_res_abs_all = (offsets_decoded - off_pred_all).abs()  # [N,Koff,3]
        mt_all = masks_decoded.to(torch.bool).expand_as(off_res_abs_all)  # [N,Koff,3]
        num_off = (off_res_abs_all * mt_all).sum(dim=(1, 2))  # [N]
        den_off = mt_all.to(torch.float32).sum(dim=(1, 2)).clamp_min(1.0)  # [N]
        off_res_mean = (num_off / den_off).unsqueeze(1)  # [N,1]
        # 预计算全局残差统计（丰富签名时会用到）
        feat_res_full = (feat_decoded - mean_all).view(N, 5, 10)  # [N,5,10]
        feat_res_mu = feat_res_full.mean(dim=2)  # [N,5]
        feat_res_std = feat_res_full.std(dim=2)  # [N,5]
        scal_res_full = (scaling_decoded - mean_scal_all)  # [N,6]
        scal_res_mu = scal_res_full.mean(dim=1, keepdim=True)  # [N,1]
        ...
        fh_dim = feat_ctx_all.shape[1]
        ctx_in = int(self.ec_head.q_enc[0].in_features)
        sig_dim_needed = max(0, ctx_in - fh_dim)

        def build_ec_context_and_bias(miss_slice, avail_idx, target_attr='feat_cc', cc=None):
            """
            固定 12 维签名：
              - feat_cc: 其他4段 feat mean/std(8) + scal mean/std(2) + off mean/std(2) = 12
              - scaling: 5段 feat mean/std(10) + off mean/std(2) = 12
              - offsets: 5段 feat mean/std(10) + scal mean/std(2) = 12
            返回: ctx_q [B,fh+12], ctx_kv [B,L,fh+12], bias_dict
            """
            B = miss_slice.numel()
            Ma = avail_idx.numel()
            if Ma == 0 or B == 0:
            ...
            # 空间 Top-L
            pos_miss = anchor_decoded[miss_slice]
            pos_av = anchor_decoded[avail_idx]
            topd_full = torch.cdist(pos_miss, pos_av)  # [B,Ma]
            Lc = min(L, Ma)
            topd, topi = torch.topk(topd_full, k=Lc, largest=False)
            nn_idx_L = avail_idx[topi]

            fh_miss = feat_ctx_all[miss_slice]  # [B,fh]
            fh_nei = feat_ctx_all[nn_idx_L.view(-1)].view(B, Lc, fh_dim)  # [B,L,fh]

            if target_attr == 'feat_cc':
                idx_others = [k for k in range(5) if k != cc]
                sig_miss = torch.cat([
                    feat_res_mu[miss_slice][:, idx_others],  # [B,4]
                    feat_res_std[miss_slice][:, idx_others],  # [B,4]
                    scal_res_mu[miss_slice], scal_res_std[miss_slice],  # [B,1], [B,1]
                    off_res_mean[miss_slice], off_res_std[miss_slice],  # [B,1], [B,1]
                ], dim=1)  # [B,12]
                sig_nei = torch.cat([
                    feat_res_mu[nn_idx_L.view(-1)][:, idx_others].view(B, Lc, 4),
                    feat_res_std[nn_idx_L.view(-1)][:, idx_others].view(B, Lc, 4),
                    scal_res_mu[nn_idx_L.view(-1)].view(B, Lc, 1),
                    scal_res_std[nn_idx_L.view(-1)].view(B, Lc, 1),
                    off_res_mean[nn_idx_L.view(-1)].view(B, Lc, 1),
                    off_res_std[nn_idx_L.view(-1)].view(B, Lc, 1),
                ], dim=2)  # [B,L,12]
            elif target_attr == 'scaling':
                sig_miss = torch.cat([
                    feat_res_mu[miss_slice], feat_res_std[miss_slice],  # [B,10]
                    off_res_mean[miss_slice], off_res_std[miss_slice],  # [B,1], [B,1]
                ], dim=1)  # [B,12]
                sig_nei = torch.cat([
                    feat_res_mu[nn_idx_L.view(-1)].view(B, Lc, 5),
                    feat_res_std[nn_idx_L.view(-1)].view(B, Lc, 5),
                    off_res_mean[nn_idx_L.view(-1)].view(B, Lc, 1),
                    off_res_std[nn_idx_L.view(-1)].view(B, Lc, 1),
                ], dim=2)  # [B,L,12]
            else:  # offsets
            ...
            ctx_q = torch.cat([fh_miss, sig_miss], dim=1)  # [B,fh+12]
            ctx_kv = torch.cat([fh_nei, sig_nei], dim=2)  # [B,L,fh+12]
            assert ctx_q.shape[1] == ctx_in, f"EC ctx dim mismatch: {ctx_q.shape[1]} vs {ctx_in}"

            # bias：距离/σ/可靠性/签名相似度（可与训练一致）
            dist = topd / (topd.mean(dim=1, keepdim=True) + 1e-9)
            if target_attr == 'feat_cc':
                sigma_nei = scale_all[nn_idx_L.view(-1), cc * 10:cc * 10 + 10].mean(dim=1).view(B, Lc)  # 简化
                conf_nei = torch.where(feat_cc_ok[cc][nn_idx_L], torch.ones_like(sigma_nei),
                                       torch.full_like(sigma_nei, float(os.getenv("HAC_EC_CONF_EC", "0.5"))))
            elif target_attr == 'scaling':
                sigma_nei = scale_scal_all[nn_idx_L.view(-1)].mean(dim=1).view(B, Lc)
                conf_nei = torch.where(scaling_ok[nn_idx_L], torch.ones_like(sigma_nei),
                                       torch.full_like(sigma_nei, float(os.getenv("HAC_EC_CONF_EC", "0.5"))))
            else:
                sigma_nei = scale_offs_all[nn_idx_L.view(-1)].view(B * Lc, -1).mean(dim=1).view(B, Lc)
                conf_nei = torch.where(offsets_ok[nn_idx_L], torch.ones_like(sigma_nei),
                                       torch.full_like(sigma_nei, float(os.getenv("HAC_EC_CONF_EC", "0.5"))))
            ...
            return ctx_q, ctx_kv, bias_dict

        def ec_head_interp_feat_cc(miss_idx_cc, cc, pred_ref):
            """用 EC 头回归 feat cc 段残差"""
            avail_idx_cc = feat_cc_ok[cc].nonzero(as_tuple=False).view(-1)
            if avail_idx_cc.numel() == 0:
                return pred_ref[miss_idx_cc]
            ctx_q, ctx_kv, bias = build_ec_context_and_bias(miss_idx_cc, avail_idx_cc, 'feat_cc', cc)
            if ctx_q is None:
                return pred_ref[miss_idx_cc]
            r_hat = self.ec_head.forward_feat_cc(ctx_q, ctx_kv, bias)  # [B,10]
            return pred_ref[miss_idx_cc] + r_hat

        def ec_head_interp_scal(miss_scal_idx, pred_scal):
            avail_scal_idx = scaling_ok.nonzero(as_tuple=False).view(-1)
            if avail_scal_idx.numel() == 0:
                return pred_scal[miss_scal_idx]
            ctx_q, ctx_kv, bias = build_ec_context_and_bias(miss_scal_idx, avail_scal_idx, 'scaling')
            if ctx_q is None:
                return pred_scal[miss_scal_idx]
            r_hat = self.ec_head.forward_scal(ctx_q, ctx_kv, bias)
            return pred_scal[miss_scal_idx] + r_hat

        def ec_head_interp_offs(miss_off_idx, pred_offs, mask_true):
            avail_off_idx = offsets_ok.nonzero(as_tuple=False).view(-1)
            if avail_off_idx.numel() == 0:
                mt = mask_true[miss_off_idx].to(pred_offs[miss_off_idx].dtype).expand_as(pred_offs[miss_off_idx])
                return pred_offs[miss_off_idx] * mt
            ctx_q, ctx_kv, bias = build_ec_context_and_bias(miss_off_idx, avail_off_idx, 'offsets')
            if ctx_q is None:
                mt = mask_true[miss_off_idx].to(pred_offs[miss_off_idx].dtype).expand_as(pred_offs[miss_off_idx])
                return pred_offs[miss_off_idx] * mt
            return self.ec_head.forward_offs(ctx_q, ctx_kv, bias, mask_true[miss_off_idx])

        fea_est = feat_decoded.clone()

        # 先准备 pred 的“因果混合均值 mu_cc”所需的 mean/scale/prob
        ...
        for cc in range(5):
            ...
            if miss_idx_cc.numel() == 0:
                # 该段没有缺失，仍需推进 fea_est 以保证后续段的因果上下文
                mean_adj_all, scale_adj_all, prob_adj_all = self.get_deform_mlp.forward(fea_est, mean_scale_cat_all,
                                                                                        to_dec=cc)
                ...
                mu_cc_all = probs_cc[..., 0] * mean_all[:, cc * 10:cc * 10 + 10] + probs_cc[..., 1] * mean_adj_all
                # 已有的 fea_est[:,cc] 保持
                continue

            # 计算 mu_cc（对所有锚点，以便与邻居计算残差）
            mean_adj_all, scale_adj_all, prob_adj_all = self.get_deform_mlp.forward(fea_est, mean_scale_cat_all,
                                                                                    to_dec=cc)
            scale_adj_all = torch.clamp(scale_adj_all, min=1e-9)
            probs_cc = torch.stack([prob_all[:, cc * 10:cc * 10 + 10], prob_adj_all], dim=-1)
            probs_cc = torch.softmax(probs_cc, dim=-1)
            mu_cc_all = probs_cc[..., 0] * mean_all[:, cc * 10:cc * 10 + 10] + probs_cc[..., 1] * mean_adj_all  # [N,10]
            ...
            if avail_idx_cc.numel() == 0:
                ...
                if not use_ec_head:
                    ...
                    rec = knn_residual_interp(miss_idx_cc, avail_idx_cc, pred_ref, actual_ref)
                else:
                    rec=ec_head_interp_feat_cc(miss_idx_cc,cc,mu_cc_all)
                fea_est[miss_idx_cc, cc * 10:cc * 10 + 10] = rec
        ...
        if miss_scal_idx.numel() > 0:
            ...
            if not use_ec_head:
                # 原 KNN 残差插值
                avail_scal_idx = scaling_ok.nonzero(as_tuple=False).view(-1)
                pred_scal = mean_scal_all  # [N,6]
                actual_scal = torch.zeros_like(pred_scal)
                actual_scal[avail_scal_idx] = scaling_decoded[avail_scal_idx]
                rec_scal = knn_residual_interp(miss_scal_idx, avail_scal_idx, pred_scal, actual_scal)
            else:
                # 学习型 EC 头
                rec_scal = ec_head_interp_scal(miss_scal_idx, mean_scal_all)

            scaling_decoded[miss_scal_idx] = rec_scal

        # offsets 缺失修复（仅 True 位）
        miss_off_idx = (~offsets_ok).nonzero(as_tuple=False).view(-1)
        if miss_off_idx.numel() > 0:
            pred_offs = mean_offs_all.view(N, self.n_offsets, 3)  # [N,Koff,3]
            mask_true = masks_decoded.to(torch.bool)  # [N,Koff,1]

            # ===== 开关：KNN 残差插值 vs EC 头 =====
            if not use_ec_head:
                # 原 KNN 残差插值
                avail_off_idx = offsets_ok.nonzero(as_tuple=False).view(-1)
                actual_offs = torch.zeros_like(pred_offs)
                actual_offs[avail_off_idx] = offsets_decoded[avail_off_idx]
                rec_offs = knn_residual_interp(miss_off_idx, avail_off_idx, pred_offs, actual_offs, mask_true=mask_true)
            else:
                # 学习型 EC 头
                rec_offs = ec_head_interp_offs(miss_off_idx, pred_offs, mask_true)

            offsets_decoded[miss_off_idx] = rec_offs
        ...
        # 尽量保留 rotation/opacity（尺寸不符时才初始化）
