    ...
    def conduct_decoding(self, pre_path_name):
        ...
        if use_gnn_inpaint:
            ...
            if num_missing > 0:
                ...
                # 构造并可选地做少量场景内自适应
                inpaintor = GNNInpaintor(
                    k=int(os.getenv("HAC_GNN_K", "16")),
                    hidden=int(os.getenv("HAC_GNN_H", "128")),
                    device='cuda'
                )
                wpth = os.getenv("HAC_GNN_WEIGHTS", "")
                if wpth and os.path.isfile(wpth):
                    try:
                        inpaintor.load_state_dict(torch.load(wpth, map_location='cuda'), strict=False)
                        print(f"[decode] Loaded GNN weights: {wpth}")
                    except Exception as e:
                        print(f"[decode] WARN: load GNN weights failed: {e}")
