...
import numpy as np
import faiss # <-- 新增
from utils.encodings import STE_multistep # <-- 新增
import torch
import torchvision
import json
import wandb
...
def training(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    # --- ↓↓↓ [!! 修改 !!] 阶段 2：EC 参数和 Faiss 初始化 ↓↓↓ ---
    logger.info("--- STARTING STAGE 2: EC TRANSFORMER TRAINING ---")

    # EC 超参数
    lambda_ec = 0.1  # EC 损失权重 (需要调整!)
    p_loss = 0.1  # 模拟丢包率 (需要调整!)
    ec_start_iter = 0  # 阶段 2 从一开始就训练 EC
    knn_batch_size = 16  # (可调整)
    ...
    try:
        ...
        print(f"Failed to initialize Faiss GPU index: {e}. KNN will fail.")
    ...
    gaussians = GaussianModel(
        ...
        # --- ↓↓↓ [!! 修改 !!] 总是传递 EC 参数 ↓↓↓ ---
        ec_model_dim=getattr(dataset, 'ec_model_dim'),  # 从 ModelParams (dataset) 获取
        ec_nhead=getattr(dataset, 'ec_nhead'),
        ec_num_encoder_layers=getattr(dataset, 'ec_num_encoder_layers'),
        ec_dim_feedforward=getattr(dataset, 'ec_dim_feedforward'),
        ec_max_neighbors=getattr(dataset, 'ec_max_neighbors'),
        ec_dropout=getattr(dataset, 'ec_dropout')
        # --- ↑↑↑ [!! 修改 !!] 修改结束 ↑↑↑ ---
    )
    ...
    # --- ↓↓↓ [!! 修改 !!] 加载并冻结 HAC++ (移除 checkpoint 恢复逻辑) ↓↓↓ ---
    if checkpoint:
        logger.warning(
            f"Ignoring --start_checkpoint '{checkpoint}'. Stage 2 always loads from --load_stage1_checkpoint.")

    load_chkpnt = getattr(opt, 'load_stage1_checkpoint', "")  # 从 opt 获取路径
    if not os.path.exists(load_chkpnt):
        raise ValueError(
            f"Stage 2 requires Stage 1 checkpoint (--load_stage1_checkpoint), but not found: {load_chkpnt}")

    logger.info(f"--- STAGE 2: Loading and Freezing HAC++ weights from {load_chkpnt} ---")
    gaussians.load_mlp_checkpoints(load_chkpnt)  # 加载阶段 1 权重

    # 冻结除 Transformer 之外的所有参数
    frozen_params = 0
    total_params = 0
    for n, p in gaussians.named_parameters():
        ...
        if 'ec_transformer' not in n:
            ...
            frozen_params += 1
    logger.info(f"Froze {frozen_params}/{total_params} parameter groups. Training only EC Transformer.")

    first_iter = 0  # 阶段 2 从头开始迭代
    # --- ↑↑↑ [!! 修改 !!] 修改结束 ↑↑↑ ---

    # --- ↓↓↓ [!! 新增 !!] 填充 Faiss 索引 (仅一次) ↓↓↓ ---
    if faiss_initialized:
        try:
            print("Populating Faiss GPU index...")
            all_anchor_pos_np = gaussians.get_anchor.detach().cpu().numpy().astype('float32')
            gpu_index.add(all_anchor_pos_np)
            torch.cuda.synchronize()
            print(f"Faiss index populated with {gpu_index.ntotal} anchors.")
        except Exception as e:
            print(f"Faiss index population failed: {e}")
            faiss_initialized = False
    # --- ↑↑↑ [!! 新增 !!] 新增结束 ↑↑↑ ---
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # ec_loss = torch.tensor(0.0, device="cuda")
        # --- ↓↓↓ [!! 新增 !!] 阶段 2 EC 损失计算 ↓↓↓ ---
        ec_loss = torch.tensor(0.0, device="cuda")

        if iteration > ec_start_iter:  # (ec_start_iter 设为 0)

            visible_anchors_indices = torch.where(voxel_visible_mask)[0]
            B = len(visible_anchors_indices)

            if B > 0:
                k_neighbors = gaussians.ec_transformer.max_neighbors

                # (1. 获取目标量化属性 real_attrs_q)
                with torch.no_grad():
                    target_pos = gaussians.get_anchor[visible_anchors_indices]
                    ...
                    # (验证断言)
                    expected_dim = (gaussians.feat_dim + 6 + 3 * gaussians.n_offsets) * 2 + gaussians.feat_dim + 3
                    actual_dim = grid_mlp_output.shape[-1]
                    split_sum = sum(split_sections)
                    assert actual_dim == expected_dim and actual_dim == split_sum, "MLP grid/split_sections mismatch!"
                    ...
                    real_feat = gaussians._anchor_feat[visible_anchors_indices]
                    real_scaling_log = gaussians._scaling[visible_anchors_indices]
                    real_offset = gaussians._offset[visible_anchors_indices]
                ...
                # (2. 随机生成丢失掩码)
                target_missing_mask = torch.rand(B, 3, device="cuda") < p_loss
                ...
                if target_missing_mask.any():
                    ...
                    all_neighbor_padding_masks = []
                    torch.cuda.synchronize()
                    print(f"Performing KNN search for {B} anchors in batches of {knn_batch_size} (using Faiss GPU)...")
                    for i in range(0, B, knn_batch_size):
                        batch_indices = visible_anchors_indices[i: i + knn_batch_size]
                        batch_neighbor_indices, batch_neighbor_padding_mask = find_k_nearest_neighbors(
                            batch_indices, gaussians.get_anchor.detach(), k=k_neighbors,
                            gpu_index=gpu_index,
                            faiss_initialized=faiss_initialized
                        )
                        all_neighbor_indices.append(batch_neighbor_indices)
                        all_neighbor_padding_masks.append(batch_neighbor_padding_mask)
                        torch.cuda.synchronize()

                    neighbor_indices = torch.cat(all_neighbor_indices, dim=0)
                    neighbor_padding_mask = torch.cat(all_neighbor_padding_masks, dim=0)
                    torch.cuda.synchronize()
                    print("KNN search completed.")
                    ...
                    with torch.no_grad():
                        ...
                        n_Q_offsets = Q_offsets_base * (
                                    1 + torch.tanh(n_Q_offsets_adj.expand(-1, 3 * gaussians.n_offsets)))
                        ...
                        neighbor_feat_q = STE_multistep.apply(neighbor_feat_orig.view(-1, gaussians.feat_dim),
                                                              n_Q_feat).view(B, k_neighbors, -1)
                        neighbor_scaling_q = STE_multistep.apply(torch.exp(neighbor_scaling_log_orig.view(-1, 6)),
                                                                 n_Q_scaling).view(B, k_neighbors, -1)
                        neighbor_offset_q = STE_multistep.apply(neighbor_offset_orig.view(-1, 3),
                                                                n_Q_offsets.view(-1, 3)).view(B, k_neighbors,
                    ...
                    if target_missing_mask[:, 0].any(): ec_loss_mask[target_missing_mask[:, 0],
                                                        :gaussians.feat_dim] = True
                    if target_missing_mask[:, 1].any(): ec_loss_mask[target_missing_mask[:, 1],
                                                        gaussians.feat_dim: gaussians.feat_dim + 6] = True
                    if target_missing_mask[:, 2].any(): ec_loss_mask[target_missing_mask[:, 2],
                                                        gaussians.feat_dim + 6:] = True

                    if ec_loss_mask.any():
                        ec_loss = l1_loss(predicted_attrs[ec_loss_mask], real_attrs_q[ec_loss_mask])

        # 阶段 2：总损失 *就是* EC 损失
        loss = lambda_ec * ec_loss
        # --- ↑↑↑ [!! 新增 !!] EC 损失计算结束 ↑↑↑ ---

        if ec_loss.item() > 0:  # (日志记录)
            if tb_writer and iteration % 100 == 0:
                tb_writer.add_scalar(f'{dataset_name}/train_loss_patches/ec_loss', ec_loss.item(), iteration)
            if wandb and iteration % 100 == 0:
                wandb.log({"train_ec_loss": ec_loss.item()})
        ...
        with torch.no_grad():
            ...
            if iteration % 100 == 0:
                # --- ↓↓↓ [修改] 进度条只显示 EC Loss ↓↓↓ ---
                postfix_dict = {"Loss": f"{ema_loss_for_log:.5f}"}
                if ec_loss.item() > 0:
                    postfix_dict["EC Loss"] = f"{ec_loss.item():.5f}"
            ...
            # # --- ↓↓↓ [修改] Log and save (移除 Ll1) ↓↓↓ ---
            # torch.cuda.synchronize();
            # t_start_log = time.time()
            # # 注意: training_report 仍会计算渲染指标 (L1, PSNR), 这是 OK 的，可以用来监控 EC 训练对渲染质量的影响
            ...
            #                 (pipe, background), wandb, logger, args_param.model_path)
            # # --- ↑↑↑ [修改] 修改结束 ↑↑↑ ---
            # Log and save
